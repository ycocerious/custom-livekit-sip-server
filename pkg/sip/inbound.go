...
func (s *Server) processInvite(req *sip.Request, tx sip.ServerTransaction) (*livekit.SIPCallInfo, rpc.IOInfoClient, error) {
	...
	log.Infow("⭐️ SIP INVITE received",
        "headers", req.Headers(),
	...
	defer func() {
		if err != nil {
			log.Infow("⭐️ SIP INVITE rejected",
				"status", "error",
				"error", err,
				"callID", callID,
			)
		} else {
			log.Infow("⭐️ SIP INVITE processed successfully",
				"callID", callID,
			)
		}
	}()
...
func (s *Server) onBye(req *sip.Request, tx sip.ServerTransaction) {
	...
	s.cmu.RUnlock()
	if c != nil {
		c.log.Infow("⭐️ SIP BYE received",
			"headers", req.Headers(),
			"source", req.Source(),
			"destination", req.Destination(),
		)
...
func (s *Server) onNotify(req *sip.Request, tx sip.ServerTransaction) {
	...
	s.cmu.RUnlock()
	if c != nil {
		c.log.Infow("⭐️ SIP NOTIFY received",
			"headers", req.Headers(),
			"body", string(req.Body()),
			"source", req.Source(),
			"destination", req.Destination(),
...
func (c *inboundCall) waitMedia(ctx context.Context) (bool, error) {
	...
	case <-c.media.Timeout():
		c.log.Infow("⭐️ SIP Media timeout occurred",
			"callID", c.callInfo.CallId,
			"fromUser", c.cc.From().User,
			"toUser", c.cc.To().User,
			"source", c.src.String(),
		)
...
func (c *inboundCall) waitSubscribe(ctx context.Context, timeout time.Duration) (bool, error) {
	...
	case <-c.lkRoom.Closed():
		...
		return false, psrpc.NewErrorf(psrpc.Canceled, "room closed")
	case <-c.media.Timeout():
		c.log.Infow("⭐️ SIP Media timeout during subscription",
			"callID", c.callInfo.CallId,
			"fromUser", c.cc.From().User,
			"toUser", c.cc.To().User,
			"source", c.src.String(),
...
func (c *sipInbound) sendRinging() {
	...
	logger.Infow("⭐️ SIP 180 Ringing sent",
		"callID", c.callID,
		"fromUser", c.From().User,
		"toUser", c.To().User,
	)
}
...
func (c *sipInbound) StartedRinging() {
	logger.Infow("⭐️ SIP call started ringing",
		"callID", c.callID,
		"fromUser", c.From().User,
		"toUser", c.To().User,
	)
...
func (c *sipInbound) Accept(ctx context.Context, sdpData []byte, headers map[string]string) error {
	...
	const hardcodedPublicContact = "Contact: <sip:43.205.152.223:5060;transport=udp>"

	// This will effectively redirect future SIP requests to this server instance (if host address is not LB).
	logger.Infow("⭐️ SIP Contact header",
		"actual_contact", c.contact,
		"new_hardcoded_contact", hardcodedPublicContact,
	)
	...
	logger.Infow("⭐️ SIP 200 OK sent",
		"callID", c.callID,
		"fromUser", c.From().User,
		"toUser", c.To().User,
		"headers", r.Headers(),
        "sdpLength", len(sdpData),
    )
